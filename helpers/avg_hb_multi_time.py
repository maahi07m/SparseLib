"""This script calculate multiplication algorithms' average execution time on hb files"""
import sys
...
def create_dicts():
    ...
    time_dict = {
        'matrix_vector': inner_dict.copy(),
        'vector_matrix': inner_dict.copy(),
        'matrix_matrix': inner_dict.copy(),
        'No_supported_format!': [],
        'Wrong_file_format': []
    }
    return time_dict
...
def read_exec_times():
    ...
    """
    times = []
    with open('../execution_results/multiplication_hb_time.txt', 'r') as f:
        for lines in f:
...
def calculate_avg(exec_time, dict_times):
    ...
    for file_name in file_names:
        if file_name in dict_times['Wrong_file_format'] or file_name in dict_times['No_supported_format!']:
            ...
            dict_times['matrix_vector'][file_name] /= 30
            dict_times['vector_matrix'][file_name] /= 30
            dict_times['matrix_matrix'][file_name] /= 30
    return dict_times
...
def create_tables(execution_time_dict):
    ...
    :return: -
    """
    workbook = xlsxwriter.Workbook('../execution_results/multiplication_hb_times_table.xlsx')
    worksheet = workbook.add_worksheet()
    caption = 'HB\'s multiplication time'

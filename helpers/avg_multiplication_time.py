"""This script calculate multiplication algorithms' average execution time """
import sys
...
def read_exec_times():
    ...
    """
    times = []
    with open('../execution_results/multiplication_time.txt', 'r') as f:
        for lines in f:
...
def calculate_avg(exec_time, dict_times):
    ...
    for time in exec_time:
        if time[0] == 'matrix-matrix':
            dict_times[time[0]][time[4]][time[1]][time[3]] += float(time[5])
        elif time[0] == 'inner_product':
            dict_times[time[0]][time[3]][time[1]] += float(time[4])
        elif time[0] == 'outer_product':
            dict_times[time[0]][time[3]][time[2]] += float(time[4])
        else:
            dict_times[time[0]][time[3]][time[1]] += float(time[4])

    for operation_type_index in dict_times:
        if operation_type_index != 'matrix-matrix':
...
def create_tables(execution_time_dict):
    ...
    :return: -
    """
    workbook = xlsxwriter.Workbook('../execution_results/multiplication_times_table.xlsx')
    worksheet = workbook.add_worksheet()
    caption = 'Multiplication execution time'
    ...
    for density, inner_dict in exec_time_dicts['matrix-matrix'].items():
        temp_list1 = [density]
        temp_list2 = [density]
    ...
    table_1_columns, table_2_columns = [{'header': 'matrix-matrix'}], [{'header': 'matrix-matrix_2n'}]
    for value in general_columns_values:

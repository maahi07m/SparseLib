"""This script calculate multiplication algorithms' average execution time on hb files using numpy"""
import sys
...
def create_dicts():
    ...
    time_dict = {
        'matrix_vector_numpy': inner_dict.copy(),
        'vector_matrix_numpy': inner_dict.copy(),
        'matrix_matrix_numpy': inner_dict.copy(),
        'No_supported_format': [],
        'Wrong_file_format': []
    }
    return time_dict
...
def read_exec_times():
    ...
    """
    times = []
    with open('../execution_results/multiplication_hb_numpy_time.txt', 'r') as f:
        for lines in f:
...
def calculate_avg(exec_time, dict_times):
    ...
    for file_name in file_names:
        if file_name in dict_times['Wrong_file_format'] or file_name in dict_times['No_supported_format']:
            ...
            dict_times['matrix_vector_numpy'][file_name] /= 30
            dict_times['vector_matrix_numpy'][file_name] /= 30
            dict_times['matrix_matrix_numpy'][file_name] /= 30
    return dict_times
...
def create_tables(execution_time_dict):
    ...
    :return: -
    """
    workbook = xlsxwriter.Workbook('../execution_results/multiplication_hb_numpy_times_table.xlsx')
    worksheet = workbook.add_worksheet()
    caption = 'HB\'s multiplication time using numpy'
    ...
    time_values_table = []
    for file_name in file_names:
        if file_name in execution_time_dict['Wrong_file_format'] or \
                file_name in execution_time_dict['No_supported_format']:
            continue
        temp_list = [file_name]
    ...
    row_to_show = 'B' + str(row_index) + ':E' + str((row_index + 166))
    row_index += 8

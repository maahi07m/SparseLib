"""This script calculate numpy's addition and subtraction algorithms' average execution time on hb files"""
import sys
...
def create_dicts():
    ...
    time_dict = {
        'addition_numpy_csr': inner_dict.copy(),
        'subtraction_numpy_csr': inner_dict.copy(),
        'addition_numpy_csc': inner_dict.copy(),
        'subtraction_numpy_csc': inner_dict.copy(),
        'No_supported_format': [],
        'Wrong_file_format': []
    }
    return time_dict
...
def read_exec_times():
    ...
    """
    times = []
    with open('../execution_results/add_sub_hb_numpy_time.txt', 'r') as f:
        for lines in f:
...
def calculate_avg(exec_time, dict_times):
    ...
    for file_name in file_names:
        if file_name in dict_times['Wrong_file_format'] or file_name in dict_times['No_supported_format']:
            ...
            dict_times['addition_numpy_csr'][file_name] /= 10
            dict_times['subtraction_numpy_csr'][file_name] /= 10
            dict_times['addition_numpy_csc'][file_name] /= 10
            dict_times['subtraction_numpy_csc'][file_name] /= 10
...
def create_tables(execution_time_dict):
    ...
    :return: -
    """
    workbook = xlsxwriter.Workbook('../execution_results/add_sub_hb_numpy_times_table.xlsx')
    worksheet = workbook.add_worksheet()
    caption = 'HB\'s addition and subtraction execution time with numpy'
